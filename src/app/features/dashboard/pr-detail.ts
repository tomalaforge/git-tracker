        ...
        <div class="flex items-start justify-between gap-4">
          <div class="min-w-0">
            <div class="flex items-center gap-2 mb-1">
              ...
              @if (pr().pr.draft) {
                <span
                  class="px-1.5 py-0.5 text-[10px] font-medium text-text-muted bg-bg-glass border border-border-glass rounded"
                  >DRAFT</span
                >
              }
            </div>
            ...
            <div class="flex items-center gap-3 mt-2 text-xs text-text-muted">
              ...
              <span
                >{{ pr().checkRuns.length }} check{{ pr().checkRuns.length !== 1 ? 's' : '' }}</span
              >
          ...
          <div class="flex flex-col items-end gap-2 shrink-0">
            ...
            <div class="flex items-center gap-1.5">
              ...
              >
                <svg
                  class="w-3.5 h-3.5"
                  [class.animate-spin-slow]="pr().isLoading"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
        ...
        @if (activeTab() === 'details') {
          ...
          <div class="p-6 space-y-5">
            <div class="space-y-1.5">
              <label class="text-[11px] font-semibold text-text-muted uppercase tracking-wider"
                >Title</label
              >
              <input
                [(ngModel)]="editTitle"
          ...
          @if (pr().isLoading) {
            ...
            <div class="p-6">
              <div class="text-center py-12">
                <div
                  class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-success-bg border border-success-border mb-4"
                >
              ...
              <div class="space-y-2 mt-4">
                @for (check of pr().checkRuns; track check.id) {
                  <div
                    class="flex items-center gap-3 px-4 py-2.5 bg-bg-glass border border-border-glass rounded-lg"
                  >
                    <svg
            ...
            <div class="p-6 text-center py-12">
              <div
                class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-pending-bg border border-pending-border mb-4"
              >
                ...
                @for (check of pr().checkRuns; track check.id) {
                  ...
                  >
                    @if (check.status === 'in_progress' || check.status === 'queued') {
                      <svg
                        class="w-4 h-4 text-pending animate-spin-slow shrink-0"
                        fill="none"
                        viewBox="0 0 24 24"
                      >
                        <circle
                    ...
                    <span class="text-sm text-text-primary">{{ check.name }}</span>
                    <span class="text-xs text-text-muted ml-auto capitalize">{{
                      check.status
                    }}</span>
                  </div>
                }
            ...
            <div class="p-6">
              ...
              <div class="space-y-4">
                @for (item of pr().failedJobs; track item.job.id) {
                  <div
                    class="bg-bg-glass border border-danger-border/30 rounded-xl overflow-hidden animate-slide-up"
                  >
                    <!-- Job header -->
                    <div
                      class="flex items-center justify-between px-4 py-3 bg-danger-bg/30 border-b border-danger-border/20"
                    >
                    ...
                    @if (!item.logAccessible) {
                      <div
                        class="px-4 py-2 bg-warning-bg/10 border-b border-warning-border/20 flex items-center gap-2"
                      >
                        <svg
                    ...
                    @if (item.testFailures.length > 0) {
                      <div class="p-4 space-y-3">
                        ...
                        @for (group of groupBySuite(item.testFailures); track group.key) {
                          <div
                            class="bg-bg-primary rounded-lg border border-border-glass overflow-hidden"
                          >
                            <!-- Suite header -->
                            <div
                              class="flex items-center justify-between px-3 py-2 bg-danger-bg/20 border-b border-border-glass"
                            >
                              <div class="flex items-center gap-2 overflow-hidden">
                                ...
                                >
                                  <svg
                                    class="w-3 h-3 group-active/copy:scale-90 transition-transform"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24"
                                  >
                                ...
                                <div class="p-3">
                                  ...
                                  <div class="space-y-0.5 mb-2">
                                    @for (
                                      ...
                                      >
                                        @if (last) {
                                          <span class="text-danger text-[11px] shrink-0 mt-px"
                                            >✕</span
                                          >
                                          ...
                                          <span class="text-text-muted text-[11px] shrink-0 mt-px"
                                            >›</span
                                          >
                                  ...
                                  @if (failure.errorMessage) {
                                    <div
                                      class="ml-3 mt-2 bg-danger-bg/30 border-l-2 border-danger/50 rounded-r-md px-3 py-2"
                                    >
                                      <pre
                                        class="text-[11px] text-danger font-mono leading-relaxed whitespace-pre-wrap break-words"
                                        >{{ failure.errorMessage }}</pre
                                      >
                                    </div>
                                  }
                                  ...
                                  @if (failure.diff) {
                                    <div
                                      class="ml-3 mt-2 bg-bg-primary rounded-md border border-border-glass overflow-hidden"
                                    >
                                      <div
                                        class="px-3 py-1.5 bg-bg-glass border-b border-border-glass"
                                      >
                                        <span
                                          class="text-[10px] text-text-muted font-semibold uppercase tracking-wider"
                                          >Expected vs Actual</span
                                        >
                                      </div>
                                      <pre
                                        class="px-3 py-2 text-[11px] font-mono leading-relaxed whitespace-pre-wrap break-words"
                                      ><!--
...
'); track $index) {<!--
                      ...
                      <div class="p-4 space-y-3">
                        ...
                        @for (annotation of item.annotations; track $index) {
                          <div
                            class="bg-danger-bg/40 border border-danger-border/30 rounded-lg p-3"
                          >
                            @if (annotation.title) {
                              <p class="text-xs font-semibold text-danger mb-1.5">
                                {{ annotation.title }}
                              </p>
                            }
                            <pre
                              class="text-xs text-text-secondary font-mono leading-relaxed whitespace-pre-wrap break-words overflow-x-auto max-h-64 overflow-y-auto"
                              >{{ annotation.message }}</pre
                            >
                            @if (annotation.path) {
                              <div
                                class="flex items-center gap-1.5 mt-2 pt-2 border-t border-border-glass text-[11px] text-text-muted"
                              >
                                <svg
                      ...
                      <div class="p-4">
                        <p
                          class="text-[10px] text-text-muted font-semibold uppercase tracking-wider mb-2"
                        >
                          Error Details
                        </p>
                        <div class="bg-bg-primary border border-border-glass rounded-lg p-3">
                          <p class="text-xs text-text-muted mb-2">
                            ...
                            <a
                              ...
                              >full job logs on GitHub</a
                            >
                            for details.
                          </p>
                          @if (getFailedSteps(item).length > 0) {
                            <p class="text-xs text-text-secondary">
                              The job failed at step
                              <span class="font-semibold text-danger"
                                >"{{ getFailedSteps(item)[0].name }}"</span
                              >
                ...
                <div class="mt-6">
                  ...
                  <div class="space-y-1.5">
                    @for (check of passingChecks(); track check.id) {
                      <div
                        class="flex items-center gap-2 px-3 py-2 bg-bg-glass border border-border-glass rounded-lg"
                      >
                        <svg
            ...
            <div class="p-6 text-center py-12">
              ...
              <p class="text-sm text-text-muted">
                This repository may not use GitHub Actions, or checks haven't been registered yet.
              </p>
            </div>
          }
        }
...
export class PrDetailComponent {
  ...
  constructor() {
    // Reset conversations and details when PR changes (real switch, not refresh)
    effect(() => {
      const pr = this.pr();
  ...
  ): { key: string; failures: ParsedTestFailure[]; suite: string }[] {
  ...
  copyCommand(group: { key: string; suite: string }): void {
    ...
    const target =
      originalTarget === 'component-test' || originalTarget === 'e2e'
        ? 'open-cypress'
        : originalTarget;
